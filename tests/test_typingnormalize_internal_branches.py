...
def test_helpers_typevar_paramspec_union_callable_origin() -> None:
    ...
    assert (
        TypingNormalize._is_callable_origin(get_origin(t.Callable[[int], int]))
        is True
    )
...
def test_string_to_type_and_forwardref_resolution_paths() -> None:
    ...
    assert TypingNormalize._resolve_forward_ref(object()) is t.Any  # type: ignore[arg-type]
...
def test_norm_callable_invalid_shape_defaults_and_forwardref_in_type() -> None:
    ...
    # a bad object and pass it through _tsub path indirectly via _from_origin check.
    # Use a mocked origin that looks like Callable but with wrong args length
    class BadCallable:
        def __getitem__(self, item):
            return ("bad", item)

    out: Type = TypingNormalize._from_origin(BadCallable(), (int,))  # type: ignore[reportInvalidTypeForm]
    assert out == ("bad", (int,))
    # ForwardRef inside Type[] resolves via _resolve_forward_ref branch
    tr: Type = TypingNormalize(t.Type[ForwardRef("int")])  # type: ignore[reportInvalidTypeForm]
...
def test_plain_runtime_to_typing_and_plain_typing_defaults_misc() -> None:
    ...
    # For typing.Mapping (not ABC), the origin name lookup goes through defaults-by-name
    assert TypingNormalize._plain_runtime_to_typing(t.Mapping) is t.Mapping
    # Regex runtime types to typing
    assert (
        repr(TypingNormalize._plain_runtime_to_typing(RePattern))
        == "typing.Pattern[typing.Any]"
    )
    # MappingView family via defaults-by-name passthrough
    assert (
        TypingNormalize._plain_typing_to_defaults(t.MappingView)
        is t.MappingView
    )
    # Callable/Union special defaults
    assert (
        repr(TypingNormalize._typing_defaults_by_name("Callable"))
...
def test_from_origin_fallback_subscription() -> None:
    ...
    out: Type = TypingNormalize._from_origin(Fake(), (int, str))  # type: ignore[reportIndexIssue]
    assert out[0] == "ok" and out[1] == (int, str)  # type: ignore[reportIndexIssue]
...
def test_from_origin_returns_origin_when_not_subscriptable() -> None:
    ...
    res: Type = TypingNormalize._from_origin(object, (int,))  # type: ignore[reportIndexIssue]
...
def test_is_concatenate_helper() -> None:
    ...
    P = ParamSpec("P")
    assert TypingNormalize._is_concatenate(Concatenate[int, P]) is True  # type: ignore[misc]
...
def test_exec_main_block_for_demo_coverage() -> None:
    ...
    src: str = open(path, "r", encoding="utf-8").read()
